...
namespace SOUI
{
	...
	void STabCtrl2::OnPaint(IRenderTarget *pRT)
	{
		...
		if(m_pSkinHeadBg)
		{
			m_pSkinHeadBg->DrawByState(pRT,rcTitle,WndState_Normal);
		}
		...
		if (m_pSkinFrame)
		{
			...
			m_pSkinFrame->DrawByIndex(pRT, rcPage, 0);
		}
	...
	void STabCtrl2::DrawItem(IRenderTarget *pRT,const CRect &rcItem,int iItem,DWORD dwState )
	{
		...
		switch(dwState)
		{
		case WndState_Normal: iState=0; break;
		case WndState_Hover: iState = 1;break;
		case WndState_PushDown :iState=2; break;
		}
		if(m_pSkinTab)
